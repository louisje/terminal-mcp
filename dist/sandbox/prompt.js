...
export async function promptForPermissions() {
    ...
    const getSelectableIndices = () => items.map((item, i) => (isSelectable(item) ? i : -1)).filter((i) => i >= 0);
    return new Promise((resolve, reject) => {
        ...
        const cycleAccess = (item, direction) => {
            if (item.type === "path") {
                const levels = ["read-write", "read-only", "blocked"];
        ...
        const formatInputAccess = (access) => {
            switch (access) {
                case "read-write":
                    return `${GREEN}Read/Write${RESET}`;
                case "read-only":
        ...
        const drawBox = (lines, title, width) => {
            ...
            output.push(`${YELLOW}${BOX.topLeft}${BOX.horizontal.repeat(leftPad)}${WHITE}${BOLD}${titleText}${RESET}${YELLOW}${BOX.horizontal.repeat(rightPad)}${BOX.topRight}${RESET}`);
            for (const line of lines) {
                const plainText = line.replace(/\x1b\[[0-9;]*m/g, "");
                const padding = width - 2 - plainText.length;
                output.push(`${YELLOW}${BOX.vertical}${RESET}${line}${" ".repeat(Math.max(0, padding))}${YELLOW}${BOX.vertical}${RESET}`);
            }
            output.push(`${YELLOW}${BOX.bottomLeft}${BOX.horizontal.repeat(width - 2)}${BOX.bottomRight}${RESET}`);
            return output.join("\n");
        };
        const cleanup = () => {
        ...
        const addCustomPath = () => {
            if (inputBuffer.trim()) {
                ...
                const customHeaderIndex = items.findIndex((i) => i.type === "header" && i.label === "Custom Paths:");
        ...
        const handleKey = (key) => {
            ...
            if (key === "\x1b[A" || key === "k") {
                ...
                if (item.type === "path" && item.custom) {
                    ...
                    if (!hasCustomItems) {
                        const headerIdx = items.findIndex((i) => i.type === "header" && i.label === "Custom Paths:");
                        if (headerIdx !== -1) {
...
function getDefaultFromItems(items) {
    const permissions = {
        filesystem: {
            readWrite: [],
            readOnly: [],
 ...
 */
export function displayPermissionsSummary(permissions) {
